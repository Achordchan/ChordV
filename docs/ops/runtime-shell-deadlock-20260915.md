...
- lib.rs / runtime.ts / App.tsx：退出清理失败发出专用事件，在主窗口显示持续的中文失败通知、原始错误及重试说明，不再只有诊断日志。
- exit-cleanup-notice.regression.ts 与双平台 workflow：执行实际通知 effect，验证错误可见、不会自动消失、含重试指引及监听清理。
- desktop check、Rust 49 项及新增回归通过；布局仅使用已有通知组件，未做截图验收。

## PR #55 第十三轮修复

- session_store.rs：已轮换的刷新令牌身份在进程生命周期内保持退役，仅保存 SHA256；晚到 IPC 的保存即使预约顺序更新，也不能恢复退役令牌。判断不受过期时间或显示字段变化影响。
- 新增轮换后再次保存旧令牌及修改元数据的回归，Rust 50 项、diff 检查通过。此节替代前述按提交顺序回收退役标记的中间实现。
