...
旧版节点的历史部署任务不作为 Go 面板参数解析。状态接口明确返回 legacy 模式并保留注册、启用和命令状态；窗口显示旧接入提示，Go 重试入口明确拒绝，不创建新任务或覆盖旧身份。超时后手动刷新若仍待注册，会清除旧超时提示并恢复等待；是否有本地安装命令决定显示 awaiting 或 resume，令牌重新生成失败的真实错误不会被后台观察静默清除。
...
- 同一状态库由进程锁保护，拒绝并行注册和重复计量。安装器另有宿主安装锁：使用经验证的 /run/chordv-agent-installer 私有 root 目录（0700），拒绝符号链接并以非截断方式打开锁文件；已接受任务通过 root 管理的 identity marker 识别重复执行。
...
在独立临时副本中执行以下十一类反向验证，均捕获预期业务错误；未把编译或测试框架错误计入通过。
...
- 恢复在 /tmp 执行下载的程序 → 实际 noexec tmpfs 上的安装成功断言失败。
- 允许服务不可见的 socket 路径 → 预检拒绝测试失败。
- 恢复在共享 /run/lock 打开截断锁文件 → 在隔离容器中，经非特权账号预置的链接截断受保护哨兵；新实现保持文件不变，并拒绝私有目录内异常链接。

**注册与界面状态**

- 去掉注册事务中的校验任务创建 → PostgreSQL 测试发现注册不再随排队失败回滚。
- 去掉服务端连接建立时的节点刷新事件 → 断线期间完成任务的重连回归失败。
- 使用旧状态接口/旧 watcher → 真实旧 Node 部署任务读取报错、旧节点状态展示断言失败。
- 恢复待注册读取后的直接返回 → 超时刷新后的等待状态恢复断言失败。
