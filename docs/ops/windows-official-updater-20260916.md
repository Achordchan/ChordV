...
安装器在标准进程检查后，以无窗口维护模式执行签名包内的新客户端，先清除 ChordV 自有代理设置，再按私有运行目录的精确路径清理遗留 Xray；不广泛强杀同名进程。清理失败会在文件安装前中止，记录诊断。
安装后保留旧程序名到当前程序的兼容硬链接（不支持硬链接时复制当前程序），保证任意已保存或固定快捷方式仍启动新版；正常启动不再删除兼容入口。Windows 安装测试覆盖自定义快捷方式、无 PID 文件的自有孤儿核心、无关同名核心与自有系统代理。
