...
状态：**实现说明与历史设计，节点改版发布验收待执行** ｜ 创建日期：2026-09-04 ｜ 状态更新：2026-09-09

> 当前交接：R3（PR #17）已合并至 main，尚不能据此判断生产升级完成。最新发布清单见节点改版 PRD 第 3 节；本文早期的 3x-ui/面板同步描述属于历史实现背景，不是当前唯一接入模式。R3 的破坏性迁移必须验证备份恢复，不能仅依赖代码回滚。
