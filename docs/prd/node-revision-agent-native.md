...
- 队列响应是**命令**不是结果(带 targetRevision),完成判定靠轮询**本命令的终态**(新增 `GET /admin/nodes/:nodeId/agent-commands/:commandId/outcome`):命令 completed 才算成功并回填;failed/cancelled 透出原因;命令行缺失(历史清理)显式失败而非猜测。节点记录的 applied revision 只用来判**被取代**——pending 且 revision 严格越过本命令 target 即失败(「已被更新的部署取代,未生效」):本次部署失败而他人更晚的部署成功时,revision 单独升高并不能证明本命令成功,对密钥轮换尤其误导(轮换没发生,后续不轮换的部署把 revision 推了过去)。轮询 3s 间隔、失败退避至 30s、5 分钟超时;超时只提示,命令仍在队列。轮询的会话纪律与接入引导钩子一致:抽屉关闭或切换节点即失效,迟到的响应/轮询不得改状态、发通知或解锁新会话的请求。
