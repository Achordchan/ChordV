...
状态:Agent 改版暂停；生产已按用户要求回退到 0.0.8 的 3x-ui 链路，详见 docs/ops/restore-xui-20260909.md

2026-09-09 接入修复（0.0.10）：0.0.9 后台更新已确认完成，但旧容器未带入 Agent/Xray 分发目录挂载和环境变量，下载返回 404。修复需同时交付安装器、Linux 双架构 Agent 包、同源 Node/Xray 分发文件，以及服务器 Compose 配置；只发布后台 tar 包不能替代这一步。

新安装器缺少兼容 Node 时自动准备 `/opt/chordv-node-runtime/v20.19.0/bin/node`，不覆盖系统 Node。Xray 使用 `/opt/chordv-xray/bin/xray` 与 `chordv-xray.service`，管理端口默认 11085，已有 ChordV 配置保留并同步给 Agent。安装不接管原 `xray.service`；若发现旧 ChordV 共用该服务，则拒绝并要求明确迁移。部署入站发现其他服务占用端口时，在配置写入及重启前拒绝，原服务不变。
更新日期:2026-09-09（初稿 2026-09-07）
当前部署证据与安装器共存边界见 `docs/ops/agent-distribution.md`。下方的 0.0.9 发布准备说明保留为历史记录，以该运维记录的最新状态为准。
当前证据:PR #17 已于 2026-09-09 合并，合并提交 `a77e4f378c7de9a1bbea4f09842de5f5e0ad317d`。用户已授权发布后台 0.0.9，并自行在面板更新；SYSTEM_VERSION 已升至 0.0.9。发布是否成功以 Release 与稳定清单一致为准；生产运行版本和迁移结果尚未核查。
...
- `trust proxy` 覆盖**两跳**代理拓扑:容器部署里 agent 的请求过 openresty(TLS 终结)→ admin 容器的 nginx → api,两跳都追加 X-Forwarded-For,agent 的地址离 socket 两项;原来固定信任一跳(且仅在生产强制 HTTPS 时生效),`request.ip` 解析到 openresty 侧地址——whoami 返回私网桥地址让 `ENSURE_INBOUND` 过不了公网校验,或把代理主机的公网地址当成节点的发布出去。默认信任私网周边(loopback/私网/CGNAT/链路本地):该拓扑的所有代理都住在那里,而 agent 从公网拨入,行走停在第一个公网地址——agent 自己伪造的 XFF 项也越不过代理合法追加的那几项。`CHORDV_API_TRUSTED_PROXIES` 可按部署收窄或替换(逗号分隔的 proxy-addr 项),`false` 只信 socket;设置改为**始终**生效,与 HTTPS 强制解耦——whoami 的观测地址依赖它。
...
- 入站部署区块挂在节点控制器抽屉(健康卡与阶段清单之间),**按 node.id 重新挂载**——抽屉在节点间复用组件,上一个节点残留的弹窗状态(表单值、已确认的密钥轮换)绝不能带到下一个节点。分两半:参数展示(接入地址/SNI/Reality 公钥/shortId/flow/fingerprint/spiderX/部署 revision,均来自 agent 回填的 `Node` 字段,`toAdminNodeRecord` 补齐了 realityPublicKey/flow/fingerprint/inboundAppliedRevision 的映射)与「部署入站 / 调整参数重新下发」。payload 携带操作员真正决定的三项——监听端口、SNI 列表(逗号分隔,**预填完整部署列表**:删掉其中一个 SNI 会让用它连接的客户端立即失效,未改动即原样重下发)、回退目标(dest,**留空按首个 SNI 同域 :443 派生**:Reality 的回退目标必须能为所选 SNI 出示有效证书,自定义 SNI 配默认 microsoft 目标能部署成功但每次握手都失败;重下发预填部署原值,自定义主机/端口不被静默替换)。**重新下发保留的是完整部署规格**——新增 `GET /admin/nodes/:nodeId/inbound-spec` 返回**最后一次 applied 的 ENSURE_INBOUND 任务 payload**(节点记录是它的有损投影:单个 serverName、无 dest、无 tag),表单预填与 preserve 都从它取——且**重新下发编辑以规格成功加载为门槛**:加载中禁用按钮、加载失败显式报错并提供重试,绝不静默退化为节点记录(那会丢多 SNI、换掉自定义回退目标);规格加载带 epoch 守卫,被取代的迟到响应不得发布;**applied revision 非零而命令行缺失**（如命令历史被清理）时显式报错而非当作未部署——那会让 UI 退化到有损节点记录;报错文案随 error 态展示。表单绑定**打开时的 applied revision**:表单开着时节点部署被变更(如另一位管理员完成部署),快照的端口/SNI/dest 即告失效,提交被阻断并提示重新打开加载当前参数——只挡「打开弹窗的按钮」不够,规格加载中/失败时提交仍可能把旧字段与回退值或新规格的保留字段拼在一起;且客户端比较**也不够**:空闲的表单永远学不到别人完成的部署(完成不推 admin 事件、钩子只在自己提交后才轮询),因此入队请求携带表单快照的 `expectedInboundAppliedRevision`,服务端在同一把 Node 行锁内做 CAS——applied revision 已越过期望值即拒绝整个入队(事务回滚同时撤销本次 revision 自增);被拒后钩子尽力拉取最新记录交给父级,过期浏览器随即看到新 revision 并被表单门槛拦下。规格随 `inboundAppliedRevision` 变化刷新——完成回调**立即合并轮询到的记录**（全表刷新只是补充：补充拉取失败时抽屉也必须拿到新 revision，否则再开表单会保留并重提旧设置）——重下发完成后节点记录刷新但 id/deployed 不变,不刷新的话再开弹窗预填的仍是上一份规格,下一次重下发会静默回滚刚完成的变更,且在下一次重新下发前必须拿到刷新后的规格;preserve 携带 flow/fingerprint/spiderX/inboundTag——这些字段若缺省,服务端的 normalizeInboundSpec 会静默重置(API 部署过 `flow: ""` 的节点会被翻成 xtls-rprx-vision,已发出的客户端配置全部断连)。首次部署不带 preserve、走控制面默认;没有 applied 任务的节点(占位符或参数来自面板导入)回退到节点记录重建(此时 inboundTag 不携带)。服务端照常归一化与校验,客户端不做二次校验逻辑。
- 队列响应是**命令**不是结果(带 targetRevision),完成判定靠轮询**本命令的终态**(新增 `GET /admin/nodes/:nodeId/agent-commands/:commandId/outcome`):命令 completed 才算成功并回填;failed/cancelled 透出原因;命令行缺失(历史清理)显式失败而非猜测。节点记录的 applied revision 只用来判**被取代**——pending 且 revision 严格越过本命令 target 时**先重读一次命令终态**再下结论:agent 按序执行命令,看到更高 revision 时本命令终态必然已提交,重读若见 completed 则按成功处理(本命令在两次读取之间完成、更晚部署又把 revision 推高的竞态下,把实际生效的轮换误报「未生效」是假阴性);仍 pending 才判失败(「已被更新的部署取代,未生效」):本次部署失败而他人更晚的部署成功时,revision 单独升高并不能证明本命令成功,对密钥轮换尤其误导(轮换没发生,后续不轮换的部署把 revision 推了过去)。轮询 3s 间隔、失败退避至 30s(重置发生在**整轮成功之后**——outcome 成功就重置会让持续失败的节点列表接口永远在 3s↔6s 之间打转)、5 分钟超时;超时只提示,命令仍在队列。轮询的会话纪律与接入引导钩子一致:抽屉关闭或切换节点即失效,迟到的响应/轮询不得改状态、发通知或解锁新会话的请求。
...
| 阶段 | 内容 | 状态 |
|---|---|---|
| R1 | 注册协议、安装与管理端接入引导 | PR #13 已合并 |
| R2 | ENSURE_INBOUND、配置管理、去重修复、管理端部署 UI | PR #14/#15/#16 已合并 |
| R3 | 3x-ui 退役、数据清理迁移、direct 计量与命令队列 | PR #17 已合并，提交 a77e4f3 |

原计划要求 R3 合并前完成生产新节点验证；实际按用户授权完成代码合并，不能把这一条标记为已验收。最后一条非阻断 P2 在 `8766bb3` 修复，用户明确要求不再复审、直接合并。其余历史审查及修复记录保留在 PR #17。

### 下一阶段：发布准备与迁移验收（未执行）

1. **先准备可验证的发布候选。** 从包含 `a77e4f3` 的 main 建立工作分支，核查控制面版本、Agent Linux 架构产物与 Xray 分发目录、安装下载路由和校验值。确认新的 SYSTEM_VERSION 后再构建；0.0.9 仅为建议的下一版本，尚未修改或发布。后台发布工作流为手动触发，合并不等于发布。
2. **先在隔离环境演练，再安排生产窗口。** 使用备份副本执行 R3 迁移，核对用户、订阅、套餐、TrafficLedger 和 SecurityEvent 的保留结果；确认 Node 及从属绑定清空符合预期。准备数据库恢复步骤，破坏性迁移后不能只回滚代码。然后在一台测试 VPS 验证安装、注册、部署入站、分配订阅、客户端真实连接、direct 入账和断网续传；同时核验节点禁用/恢复与命令失败提示。
3. **获得发布与服务器操作授权后实施生产迁移。** 先核查实际版本、备份、维护窗口及面板目录，再更新控制面、按核实后的方案处理旧 VPS 服务、重建节点和订阅分配、重发订阅，并逐项记录第 4 节验收证据。生产未核查、未迁移、未验收的项目保持未完成。

当前没有定义 R4 功能开发范围。兼容枚举、旧命名及历史迁移核对工具不作为此次发布前的无条件清理任务；若需移除，先确认调用与历史数据兼容性。
