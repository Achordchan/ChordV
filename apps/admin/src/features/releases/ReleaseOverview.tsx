...
export function ReleaseOverview(p:Props) {
  ...
  return <section className={styles.overview}>
    ...
    <div className={styles.table}><DataTable minWidth={900}><Table.Thead><Table.Tr>{["版本与平台","安装包","状态","更新时间","操作"].map(t=><Table.Th key={t}>{t}</Table.Th>)}</Table.Tr></Table.Thead><Table.Tbody>{records.length?records.map(r=><Fragment key={r.id}><Table.Tr>
      ...
      <Table.Td><Text size="sm" c={r.artifacts.length?undefined:"orange"}>{r.artifacts.length?`${r.artifacts.length} 个安装包`:"待添加安装包"}</Text>{r.artifacts.length>0?<Text size="xs" c="dimmed" mt={5}>{r.artifacts.some(a=>a.source==="external")?"含旧外链":"本站托管"}</Text>:null}</Table.Td>
      ...
      <Table.Td><Group gap="xs" wrap="nowrap">{r.status==="draft"?<Button size="xs" color="teal.9" variant={r.artifacts.length?"light":"filled"} disabled={p.busy} onClick={()=>r.artifacts.length?p.onPublish(r):p.onAdd(r)}>{r.artifacts.length?"发布版本":"添加安装包"}</Button>:null}<Menu withinPortal position="bottom-end"><Menu.Target><ActionIcon variant="subtle" color="gray" aria-label="发布操作" disabled={p.busy}><IconDots size={17}/></ActionIcon></Menu.Target><Menu.Dropdown><Menu.Item onClick={()=>setExpanded(expanded===r.id?null:r.id)}>查看详情</Menu.Item>{r.status!=="archived"?<><Menu.Item onClick={()=>p.onEdit(r)}>编辑版本信息</Menu.Item><Menu.Item onClick={()=>p.onAdd(r)}>管理安装包</Menu.Item>{r.status!=="draft"?<Menu.Item onClick={()=>p.onWithdraw(r)}>撤回为草稿</Menu.Item>:null}<Menu.Item color="red" onClick={()=>p.onDelete(r)}>删除发布</Menu.Item></>:null}</Menu.Dropdown></Menu></Group></Table.Td>
    </Table.Tr>{expanded===r.id?<Table.Tr><Table.Td colSpan={5}><div className={styles.expanded}><section><h3>更新说明</h3>{r.changelog.length?<ul>{r.changelog.map((line,i)=><li key={i}>{line}</li>)}</ul>:<Text size="sm" c="dimmed">尚未填写更新说明</Text>}</section><section><h3>安装包</h3>{r.artifacts.map(a=><div key={a.id} className={styles.artifact}><Group justify="space-between"><div><Text size="sm" fw={550}>{a.fileName||"外链安装包"}</Text><Text size="xs" c="dimmed">{a.source==="external"?"旧外链":"本站托管"} · {a.type.toUpperCase()}{a.isPrimary?" · 更新入口":""}</Text></div><Group gap="xs"><Button variant="subtle" size="xs" onClick={()=>p.onCopy(a.downloadUrl)} leftSection={<IconLink size={14}/>}>复制地址</Button>{r.status==="draft"?<><Button variant="subtle" size="xs" disabled={p.busy} onClick={()=>p.onEditArtifact(r.id,a)}>编辑</Button><Button variant="subtle" color="red" size="xs" disabled={p.busy} onClick={()=>p.onDeleteArtifact(r.id,a.id)}>删除</Button></>:null}</Group></Group><details><summary>地址与校验信息</summary><p>{a.downloadUrl}</p><p>文件大小：{a.fileSizeBytes||"未填写"} 字节</p><p>SHA256：{a.fileHash||"未填写"}</p></details></div>)}{!r.artifacts.length?<Text size="sm" c="dimmed">草稿已保存，可以继续远程获取或上传安装包。</Text>:null}</section></div></Table.Td></Table.Tr>:null}</Fragment>):<Table.Tr><Table.Td colSpan={5}><Text ta="center" c="dimmed" py="xl">没有符合条件的发布记录</Text></Table.Td></Table.Tr>}</Table.Tbody></DataTable></div>
  </section>;
}
