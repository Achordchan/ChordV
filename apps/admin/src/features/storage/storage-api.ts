...
export async function scanStorage(signal:AbortSignal,onProgress:(message:string)=>void) {
  const boundedSignal = AbortSignal.any([signal, AbortSignal.timeout(10*60_000)]);
  const response=await requestResponse("/admin/storage/scan",{method:"POST",signal:boundedSignal,timeoutMs:10*60_000});
