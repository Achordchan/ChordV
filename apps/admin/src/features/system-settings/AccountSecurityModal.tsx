...
export function AccountSecurityModal(props: Props) {
  ...
  return <Modal opened={props.opened} onClose={props.onClose} title="账号安全" centered size={520}
    ...
    <form className={`${dialog.form} ${styles.security}`} onSubmit={event=>{event.preventDefault();if(!props.saving && !mismatch && (!changePassword || (props.form.newPassword.length >= 8 && Boolean(props.form.confirmPassword))) && event.currentTarget.checkValidity())props.onSave();}}>
