...
function config(file: string): AgentConfig {
  return { credentialsPath: file, registerToken: 'chordv_register_fixture', agentId: '', nodeId: '', token: '',
    ...
    databasePath: join(dirname(file), 'agent.db'), inboundRequestDir: join(dirname(file), 'xray'), inboundResultDir: join(dirname(file), 'xray-out'), restartToleranceMs: 2_000, sampleIntervalMs: 5000, heartbeatIntervalMs: 15000, offlineAllowanceBytes: 1n };
}
