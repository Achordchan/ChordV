...
function config(root: string, extra: Partial<AgentConfig> = {}): AgentConfig {
  return { credentialsPath: join(root, 'credentials.json'), registerToken: 'chordv_register_health', agentId: '', nodeId: '', token: '',
    ...
    databasePath: join(root, 'agent.db'), inboundRequestDir: join(root, 'xray'), inboundResultDir: join(root, 'xray-out'), restartToleranceMs: 2_000, sampleIntervalMs: 5000, heartbeatIntervalMs: 15000, offlineAllowanceBytes: 1n, ...extra };
}
