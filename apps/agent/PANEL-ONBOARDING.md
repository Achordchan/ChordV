...
> **交付状态更正（2026-09-10）：一键接入尚未完成，暂不作为新版发布/用户验收流程。**
> 下文的手工安装只是已有实现的调试路径，不满足用户要求；旧 Node/Xray 安装入口仍需替换。
> 接手任务与验收标准以 [HANDOFF-ONE-CLICK.md](HANDOFF-ONE-CLICK.md) 为准。
...
1. 在目标机器确认 3x-ui **3.1.0 及以上的 3.x 稳定版**。手工填写的后台版本为管理员声明，**不是自动探测结果**；
